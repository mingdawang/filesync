...
- 仅支持单向同步（源 → 目标）
- 支持镜像模式（删除目标端孤立文件）和保留模式（保留目标端多余文件）两种策略
...
- **单向模式（One-Way，默认）**：只将源端文件同步到目标端，不删除目标端多余文件（孤立文件）
- 每个任务独立配置同步模式
...
- 间隔时间：1 分钟 ~ 几天（以分钟为单位输入）
- 程序运行期间自动检查，无需外部任务计划程序
- 任务列表中显示下次执行时间（⏰ HH:MM）
- 从未执行过的任务：启动后立即执行一次
...
- **文件比较方式**：元数据（大小 + 修改时间，默认）/ 内容哈希（BLAKE3，精确但较慢）
- **界面主题**：跟随系统 / 浅色 / 深色
- **新建任务默认并发数**：1–16（默认 4）
...
- 最小窗口尺寸：900 × 600 px，默认启动尺寸：1280 × 800 px
...
│       ├── compare_method: CompareMethod // Metadata | Hash
...
├── sync_mode: SyncMode                  // Mirror（删孤立）| OneWay（保留孤立，默认）
...
1. 目标文件不存在
2. 文件大小不同
3. 源文件修改时间 > 目标文件修改时间（允许 2 秒误差，兼容 FAT32 时间戳精度）
...
文件需要复制（Create 或 Update）？
    ...
    │       ├── 大文件（>= unbuffered_threshold，默认 128MB，本地卷）
    ...
    └── 文件大小 >= delta_threshold 且目标文件存在（Update）
            └── → 差量传输（Delta Sync，rsync 算法）
                    └── 失败或差量 > 80% → 降级为 CopyFileEx
```
...
Step 2: 差异计算（Diff）
  ...
  └── Hash 模式：
      ├── 对 Update 条目计算 BLAKE3 哈希
      ├── 若 USN 跳过优化可用：src_frn 和 dst_frn 均不在变更集 → 直接降级为 Skip
      └── 哈希相同 → 降级为 Skip
...
Step 4: 执行（Execute）
  ...
  └── 镜像模式：删除孤立文件（Orphan），发送 FileDeleted 事件
...
Step 2: 源端滚动扫描
  ...
  每个位置计算 Adler-32 → 查哈希表 → 命中则校验 BLAKE3 确认
    命中（match）  → 记录"引用目标块 #N"（无需传输）
    未命中（literal）→ 记录"原始字节数据"（需写入）
...
| 文件大小 | 块大小 | 说明 |
|----------|--------|------|
| 4–50 MB | 2 KB | 小文件，小块提高命中率 |
| 50–500 MB | 16 KB | 中等文件，平衡内存和命中率 |
| 500 MB–5 GB | 64 KB | 大文件，减少哈希表内存占用 |
| > 5 GB | 128 KB | 超大文件，最小化内存占用 |
...
```
%APPDATA%\FileSync\config.json
```

示例：`C:\Users\<用户名>\AppData\Roaming\FileSync\config.json`
...
│   └── app.rc
└── src/
    ├── main.rs                      // 程序入口，初始化 eframe，程序化生成图标
    ├── app.rs                       // 顶层 App 状态，实现 eframe::App trait
    ...
    │   ├── executor.rs              // 并发执行（tokio + Semaphore）+ USN 预扫描
    │   ├── copier.rs                // 文件复制策略（CopyFileEx / 无缓冲 IO）
    │   ├── delta.rs                 // 差量传输（rsync 算法：Adler-32 + BLAKE3）
    │   ├── hash.rs                  // 文件哈希工具（BLAKE3）
    │   └── events.rs                // 进度事件定义（SyncEvent enum）
    │
    ├── fs/                          // 文件系统特性封装
    │   ├── mod.rs
...
- [x] 大文件无缓冲 IO
- [x] 卷类型探测（`fs/volume.rs`，基础版）
- [ ] 稀疏文件感知复制（计划中）
- [ ] ReFS Block Clone（计划中）
- [ ] Move/Rename 检测（计划中）
